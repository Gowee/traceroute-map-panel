...
import 'panel.css';
import { pointsToBezierPath1 } from 'bezierCurve';
  ...
  render() {
    ...
    return (
      ...
      >
        <style type="text/css">
          {`
        .host-tray .host-label, .host-tray .dest-label {
        ...
        <Control position="bottomleft">
          {this.state.hostListExpanded ? (
            <>
              <span
                className="host-tray-toggler host-tray-collapse"
                title="Collapse hosts list"
                onClick={() => this.toggleHostList()}
              >
                <i className="fa fa-compress" />
              </span>
              <ul className="host-tray">
                {Array.from(data.entries()).map(([key, points]) => {
            ...
            <span
              className="host-tray-toggler host-tray-expand"
